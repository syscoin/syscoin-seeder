...
bool TestNode(const CService &cip, int &ban, int &clientV, std::string &clientSV, int &blocks, vector<CAddress>* vAddr) {
  try {
    ...
    if (!ret) {
      ...
      else if(clientSV.find("/Satoshi:4.3.0/") == std::string::npos && clientSV.find("/Satoshi:4.3.0rc1/") == std::string::npos){
        ret = 0;
        ban = 1;     
      }
      else if(clientSV.find("/Satoshi:4.3.0rc1/") != std::string::npos && blocks < 840000){
        ret = 0;
        ban = 1;
      }
      else if(clientSV.find("/Satoshi:4.3.0/") != std::string::npos && blocks < 1348000){
        ret = 0;
        ban = 1;
      }
      else {
...
/*
int main(void) {
  CService ip("syscoin.org", 8369, true);
  vector<CAddress> vAddr;
  vAddr.clear();
  int ban = 0;
